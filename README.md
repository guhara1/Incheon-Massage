...
- **전용 페이지 링크아웃**: 예약 시간·준비물·위생·요금은 동/역 본문에서 반복하지 않고
  `/incheon/hours·checklist·safety/`, `/course/price/`로 링크만 겁니다(반복 블록 제거 → 유사도 감소).
...
- 도어웨이 유사도(4-gram 자카드, 목표 ≤ ~40%): 대표 동 **39%**, 구·군 **29%**, 테마 **40%**, 역 **44%**.
  역 그룹은 공통 링크아웃 섹션 비중으로 다소 높으나, 각 역은 노선·환승·구·랜드마크·평균 도착 등 고유 데이터를 가집니다.
